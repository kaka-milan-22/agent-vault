...
// UNVAULTED placeholders carry a sha256 prefix used to look up the original
// value during write-back. Output uses UNVAULTED_PREFIX_LEN hex chars; the
// regex accepts 8..16 so legacy 8-char placeholders generated by older
// versions still round-trip.
const UNVAULTED_PREFIX_LEN = 12;
const UNVAULTED_RE = /<agent-vault:UNVAULTED:sha256:([a-f0-9]{8,16})>/g;
...
function sha256Prefix(s: string, len: number = UNVAULTED_PREFIX_LEN): string {
  return sha256Hex(s).slice(0, len);
}
...
export function restoreUnvaulted(
  ...
  // Collect candidate values from the existing file with their full sha256.
  // Matching is done by slicing the full hash to the placeholder's actual
  // prefix length, so 8-char (legacy) and 12-char (current) placeholders
  // both resolve correctly.
  const candidates: Array<{ value: string; fullHash: string }> = [];
  const seen = new Set<string>();
  const existingLines = existingContent.split("\n");

  for (const line of existingLines) {
    const lineCandidates = extractValueCandidates(line);
    for (const candidate of lineCandidates) {
      const trimmed = candidate.trim().replace(/^["']|["'],?$/g, "");
      if (trimmed.length < HIGH_ENTROPY_MIN_LENGTH) continue;
      if (seen.has(trimmed)) continue;
      if (matchesSecretPattern(trimmed) || (isHighEntropy(trimmed) && !looksLikeNonSecret(trimmed))) {
        candidates.push({ value: trimmed, fullHash: sha256Hex(trimmed) });
        seen.add(trimmed);
      }
    }
  }
  ...
  const result = content.replace(UNVAULTED_RE, (match, hash: string) => {
    const len = hash.length;
    const found = candidates.find((c) => c.fullHash.slice(0, len) === hash);
    if (found) {
      restoredCount++;
      return found.value;
    }
